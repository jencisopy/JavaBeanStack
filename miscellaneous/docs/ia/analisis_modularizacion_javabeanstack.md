...
| 7 | Overrides por classpath → puntos de extensión | 🟡 Parcial | La selección de `IDBManager` (clásico/V20/V30; V21 eliminado el 2026-07-12) vía `ejb-jar.xml` del consumidor quedó consolidada y **documentada** como punto de extensión (ver README de `miscellaneous/docs/ia`). Siguen los overrides por classpath en Maker: dialectos Hibernate, `org.javabeanstack.log`, `py.com.oym.frame.security`. (`appcatalog` **ya no cuenta como override**: es implementación de interfaz por diseño — ver punto 2.) **Fuera del alcance de esta iteración** |
...
| R7 | Puntos de extensión para log/seguridad/dialectos (patrón `ejb-jar.xml` del DBManager) | 1–2 semanas | Medio | ❌ Pendiente. Elimina los overrides por classpath restantes (ya no incluye `appcatalog`) |
