...
public abstract class AbstractDataObject<T extends IDataRow> implements IDataObject, Serializable {
    ...
    protected void setSelectcmd() {
        ...
        if (isFieldExist("idempresa") && getIdempresa() != 0) {
            //TODO ver para aplicar IDbFilter()
    ...
    public boolean goTo(int rownumber, int offset) {
        ...
        while (true) {
            ...
            if (!showDeletedRow && dataRows.get(rownumber).getOperation() == IDataRow.BORRAR) {
    ...
    public static int findRow(List<IDataRow> rowList, String field, Object value, int begin, int end) {
        ...
        for (int i = begin; i <= end; i++) {
            ...
            if (row.getOperation() != IDataRow.BORRAR) {
    ...
    public boolean find(String field, Object value, int begin, int end) {
        ...
        int rec = DataObject.findRow((List<IDataRow>) dataRows, field, value, begin, end);
        if (rec >= 0) {
            this.goTo(rec);
    ...
    public boolean setField(String fieldname, Map param) {
        try {
            ...
            obj = getDAO().findByUk((Class<IDataRow>) obj.getClass(), obj);
    ...
    public boolean refreshRow() {
        if (row == null || row.getOperation() == IDataRow.AGREGAR) {
            return false;
        }
        try {
            errorApp = null;
    ...
    public boolean insertRow() {
        ...
        try {
            ...
            newRow.setOperation(IDataRow.AGREGAR);
    ...
    public Map<String, IErrorReg> checkDataRow() throws Exception {
        ...
        Map<String, IErrorReg> errorMap = this.getDAO().getDataService().checkDataRow(row, sessionId);
        ...
        try {
            ...
            if (allRows) {
                ...
                if (!dataResult.isRemoveDeleted()
                        && row.getOperation() == IDataRow.BORRAR) {
                    removeRow();
                }
            }
    ...
    public boolean update(IDataSet dataSet) {
        ...
        try {
            ...
            initDataRowsBak();
            // Eliminar de la lista local el registro actual si esta marcado para ser borrado
            if (!dataResult.isRemoveDeleted() && row.getOperation() == IDataRow.BORRAR) {
                removeRow();
            }
    ...
    public boolean revert(Boolean allRows) {
        if (allRows) {
            ...
            while (dataRows.size() > i) {
                T r = dataRows.get(i);
                if (r.getOperation() > 0) {
                    if (r.getOperation() == IDataRow.AGREGAR) {
                        dataRows.remove(i);
                        continue;
                    } else {
                        goTo(i);
